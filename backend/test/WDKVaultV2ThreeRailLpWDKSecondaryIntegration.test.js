...
describe("OmniAgentVault V2 — Three-Rail LP Integration", function () {
  async function deployThreeRailFixture() {
    ...
    const stableSwapPool = await MockStableSwapPoolWithLPSupport.deploy(
      wdks.target,
      usdt.target, // token0=WDKS (index 0), token1=USDT (index 1)
    ...
    // normalWDKBps=2000 + normalLpBps=2000 = 4000 <= 9000 ✓
    ...
    await wdks.mint(stableSwapPool.target, ethers.parseUnits("5000000", 6));
  ...
  describe("Deployment & Wiring", function () {
    ...
    it("Should lock vault and all adapters", async function () {
      const { vault, wdkAdapter, secondaryAdapter, lpAdapter } =
    ...
    const policy = await RiskPolicy.deploy(
      ...
      const minter = await MockAsyncWDKMinter.deploy(usdt.target, 3600);
      ...
      const wdk = await WDKEarnAdapter.deploy(
        usdt.target,
        minter.target,
        depositSel,
        managedAssetsSel,
      ...
      const chainlinkFeed = await MockChainlinkAggregator.deploy(8, 100000000n);
      const wdks2 = await MockERC20.deploy("WDKS", "WDKS");
      ...
      await vault.setEngine(engine.target);
      // Only set wdk + secondary, no LP — lpAdapter is optional so should succeed
      await vault.setAdapters(
        wdk.target,
        secondary.target,
        ethers.ZeroAddress,
  ...
  describe("LP Allocation Per Risk State", function () {
    it("Normal state: wdkBps=2000, lpBps=2000", async function () {
      const { engine } = await loadFixture(deployThreeRailFixture);
      const preview = await engine.previewDecision();
      expect(preview.nextState).to.equal(0); // Normal
      expect(preview.targetWDKBps).to.equal(2000n);
      expect(preview.targetLpBps).to.equal(2000n);
    });

    it("Guarded state: wdkBps=5000, lpBps=1500", async function () {
      ...
      expect(preview.targetWDKBps).to.equal(5000n);
      expect(preview.targetLpBps).to.equal(1500n);
    });

    it("Drawdown state: wdkBps=7000, lpBps=500", async function () {
      const { engine, oracle } = await loadFixture(deployThreeRailFixture);
  ...
  describe("RiskPolicy LP Param Validations", function () {
    it("Should revert if normalLpBps + normalWDKBps > 10000", async function () {
      const RiskPolicy = await ethers.getContractFactory("RiskPolicy");
      await expect(
        RiskPolicy.deploy(
  ...
  describe("Liquidity Waterfall (4-tier)", function () {
    it("Should pull from LP when secondary is drained", async function () {
      const { vault, engine, lpAdapter, wdkAdapter, secondaryAdapter, user, executor, usdt } =
        await loadFixture(deployThreeRailFixture);
      const amount = ethers.parseUnits("10000", 6);
      await vault.connect(user).deposit(amount, user.address);
