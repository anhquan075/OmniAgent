...
class AutonomousLoopService:
    ...
    async def start(cls, app: FastAPI) -> None:
        ...
        if not settings.bnb_autonomous_loop_enabled:
            ...
            logger.bind(event="autonomous_loop_disabled", **status).info("backend autonomous loop disabled")
        ...
        logger.bind(event="autonomous_loop_started", **status).info("backend autonomous loop started")
    ...
    async def stop(cls, app: FastAPI) -> None:
        ...
        try:
            ...
            cls.set_status(app, status)
            logger.bind(event="autonomous_loop_stopped", **status).info("backend autonomous loop stopped")
        setattr(app.state, cls.task_state_key, None)
    ...
    async def run_forever(cls, settings: Settings, app: FastAPI | None = None) -> None:
        ...
        while True:
            ...
            cls.set_status(app, cls.status_payload(settings, state="running", lastRunAt=started_at))
            try:
                ...
                cls.set_status(app, status)
                logger.bind(event="autonomous_loop_cycle_failed", **status).exception(
                    "backend autonomous loop cycle failed"
                )
            else:
                next_run_at = cls.iso_after(interval)
                status = cls.status_payload(
