...
async function main() {
  ...
  let deployOutput;
  try {
    deployOutput = execSync(`${hhBin} run scripts/DeployWDKStack.js --network localhost`).toString();
  ...
  const tx1 = await bnbAccount.sendTransaction({ to: usdtAddr, value: 0n, data: approveData });
  console.log("tx1:", tx1);
  await new Promise(r => setTimeout(r, 2000));
  
  const depositData = vaultIface.encodeFunctionData("deposit", [agentUsdtBal, agentAddr]);
  const tx2 = await bnbAccount.sendTransaction({ to: vaultAddr, value: 0n, data: depositData });
  console.log("tx2:", tx2);
  await new Promise(r => setTimeout(r, 2000));
