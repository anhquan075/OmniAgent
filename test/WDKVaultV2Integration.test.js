...
describe("WDKVault V2 Integration", function () {
  async function deployV2Fixture() {
    ...
    const MockWDKSMinting = await ethers.getContractFactory("MockWDKSMinting");
    const wdksMinting = await MockWDKSMinting.deploy(usdt.target, wdks.target);
    ...
    // Compute selectors for WDKEarnAdapter
    const depositSel = wdkMinter.interface.getFunction("deposit").selector;
    const managedAssetsSel = wdkMinter.interface.getFunction("managedAssets").selector;
    const requestWithdrawSel = wdkMinter.interface.getFunction("requestWithdraw").selector;
    const claimWithdrawSel = wdkMinter.interface.getFunction("claimWithdraw").selector;
    const getWithdrawRequestSel = wdkMinter.interface.getFunction("getWithdrawRequest").selector;

    // Deploy adapters
    const WDKEarnAdapter = await ethers.getContractFactory("WDKEarnAdapter");
    const wdkAdapter = await WDKEarnAdapter.deploy(
      usdt.target, wdkMinter.target,
    ...
    const pegArb = await PegArbExecutor.deploy(
      vault.target, usdt.target, wdks.target, wdksMinting.target, stableSwapPool.target,
      10, 500, 50, 50
    );
    ...
    await usdt.mint(wdksMinting.target, ethers.parseUnits("10000000", 18));
    await usdt.mint(stableSwapPool.target, ethers.parseUnits("1000000", 18));
    await wdks.mint(stableSwapPool.target, ethers.parseUnits("1000000", 18));
    ...
    return {
      ...
      usdt, wdks, vault, engine, policy, oracle, breaker, sharpeTracker,
      wdkAdapter, secondaryAdapter, lpAdapter, wdkMinter, pegArb,
      chainlinkFeed, stableSwapPool, wdksMinting
    };
  }

  describe("Deployment & Configuration", function () {
    it("Should wire all contracts correctly", async function () {
      const { vault, engine, wdkAdapter, secondaryAdapter, lpAdapter } = await loadFixture(deployV2Fixture);
      expect(await vault.engine()).to.equal(engine.target);
      expect(await vault.wdkAdapter()).to.equal(wdkAdapter.target);
    ...
    it("Should lock configuration and zero owner", async function () {
      const { vault, wdkAdapter, secondaryAdapter, lpAdapter } = await loadFixture(deployV2Fixture);
      expect(await vault.configurationLocked()).to.be.true;
      expect(await wdkAdapter.configurationLocked()).to.be.true;
  ...
  // Async WDK Withdrawal tests removed - adapters are internal vault components with onlyVault modifier
