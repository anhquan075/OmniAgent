...
export const TaskStep = ({ label, status, details }: TaskStepProps) => {
  ...
  return (
    <Task defaultOpen={status === 'in-progress'} className="w-full">
      <div className={clsx(
        "flex flex-col gap-1 p-2 sm:p-3 rounded-xl border transition-all duration-300",
        status === 'in-progress' ? "bg-tether-teal/10 border-tether-teal/30 shadow-glow-sm" : "bg-white/5 border-white/5"
      )}>
